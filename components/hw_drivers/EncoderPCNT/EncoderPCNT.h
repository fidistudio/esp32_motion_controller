...
enum class VelocityUnits : uint8_t { RPM, RAD_S };

/* ================= PCNT ================= */

class PCNTPulse {
public:
  using isr_cb_t = bool (*)(pcnt_unit_handle_t, const pcnt_watch_event_data_t *,
                            void *);
  PCNTPulse(gpio_num_t pulse_gpio, isr_cb_t callback, void *callback_arg,
...
public:
  EncoderPulseSource(gpio_num_t pulse_gpio, gpio_num_t sector0_gpio,
...
private:
  static bool IRAM_ATTR onPulse(pcnt_unit_handle_t,
                                const pcnt_watch_event_data_t *, void *arg);
  static bool IRAM_ATTR onSector0(pcnt_unit_handle_t,
                                  const pcnt_watch_event_data_t *, void *arg);
...
public:
  ...
  /**
   * @brief Posición angular acumulada desde arranque en radianes.
   *
   * Incrementa +RAD_PER_PULSE en cada pulso si gira hacia adelante,
   * -RAD_PER_PULSE si está invertido (reversa).
   * Puede crecer indefinidamente — compatible con sensor_msgs/JointState.
   */
  float getPosition();

  /**
   * @brief Reinicia el acumulador de posición a cero.
   * Útil si quieres resetear la odometría desde data.cpp o ROS.
   */
  void resetPosition();
...
private:
  ...
  static constexpr float RAD_PER_PULSE_ = (2.0f * 3.14159265f) / NUM_SECTORS_;
