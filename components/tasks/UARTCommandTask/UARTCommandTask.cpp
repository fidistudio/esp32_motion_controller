...
static void UARTCommandTask(void *arg) {
  ...
  while (1) {
    int len =
        uart_read_bytes(UART_PORT, rx_buf, sizeof(rx_buf), pdMS_TO_TICKS(100));
      ...
      if (c == '\n' || c == '\r') {
        if (line_overflow) {
          ...
          if (strncasecmp(line_buf, "duty:", 5) == 0) {
            ...
            float left, right;
            int parsed = sscanf(arg, "%f %f", &left, &right);

            if (parsed != 2) {
              ESP_LOGW(TAG, "Formato inválido. Use: duty: <left> <right>");
            } else {
              setDuty(left, right);
              ESP_LOGI(TAG, "DUTY recibido L=%.2f R=%.2f", left, right);
            }
          }
          ...
          // ===== TWIST linear angular =====
            ...
            int parsed = sscanf(arg, "%f %f", &linear, &angular);
